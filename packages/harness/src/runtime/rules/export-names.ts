...
import { bundledPackages } from '../../core/workspace.ts';
...
const rule: AuditRule = {
  ...
  run({ root }) {
    ...
    const files = bundledPackages(root).flatMap((entry) => {
      const sources = path.join(entry.dir, 'src');
      return fs.existsSync(sources)
        ? sourceFiles(sources, '').map((file) => path.relative(root, path.join(sources, file)))
        : [];
    });
