...
export function auditSurfaceContrast(
  color: ColorLookup,
  add: AddFinding,
  contract: Pick<DesignSystemContract, 'focus' | 'pageGround' | 'surfaceContrast'>,
): void {
  const { controls, disabled, focusGrounds, shadows, textPairs } = contract.surfaceContrast;
  const page = color(contract.pageGround);
  if (!page || (page.a != null && page.a < 1)) {
    add('FAIL', '1.4.11', 'page ground', contract.pageGround + ' must be an opaque colour token');
    return;
  }
  const pageColor: Color = { r: page.r, g: page.g, b: page.b };
  const opaque = (token: string): Color | undefined => {
    const value = color(token);
    return value && flatten(value, pageColor);
  };
