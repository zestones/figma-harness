...
function worker(packageManager: string, task: string, args: string[]): void {
  ...
  if (task === 'test') {
    ...
    for (const file of args) {
      if (!/^(?:[\w-]+\/)+tests\/[\w-]+\.test\.ts$/.test(file) || !existsSync(path.join(ROOT, file))) {
        throw new Error('Expected an existing <package>/tests/<name>.test.ts selector');
      }
    }
    executable = process.execPath;
    command = [`--max-old-space-size=${heapMb}`, '--import', 'tsx', '--test', '--test-concurrency=1', ...args];
  } else {
    [executable, command] = packageManagerCommand(packageManager, ['run', task, ...args]);
  }
  const result = spawnSync(executable, command, { cwd: ROOT, stdio: 'inherit' });
  reportPeak();
...
function main(): void {
  ...
  if (input[0] === '--worker') {
    const [, packageManager, task, ...args] = input;
