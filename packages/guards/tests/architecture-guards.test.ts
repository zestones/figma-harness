...
const miniWorkspace = function (files: Record<string, string>): string {
  ...
  const all: Record<string, string> = {
    'figma-harness.config.json': JSON.stringify({ plugin: 'plugin', designSystem: 'design-systems/kit', renders: 'renders' }),
...
test('the graph validator reports bypasses, undeclared packages and JavaScript sources', () => {
  const root = miniWorkspace({
    ...
    'apps/demo/src/pages/page.ts': [
      ...
      'export const page = [mechanics, kit, system, engine, plugin, figma];',
      '',
    ].join('\n'),
  });
...
test('the authoring policy protects task scope and baselines', () => {
  ...
  assert.match(
    validateChangedPaths(['plugin/baselines/design.json'], 'STRUCTURAL_MAINTENANCE', policy)[0],
    /BASELINE_ACCEPTANCE/,
  );
  assert.deepEqual(validateChangedPaths(['plugin/baselines/design.json'], 'BASELINE_ACCEPTANCE', policy), []);
});
