/* Matrix-driven prototype planning; native I/O stays behind the public kit. */
  ...
  return {
    ...
    actions: [{
      ...
      transition: transition.motion
        ? prototypeMotionTransition(transition.motion)
        : null,
  ...
  for (const transition of flows.transitions) {
    ...
    for (const sourceKey of transition.sources) {
      ...
      for (const node of selectNodes(source, sourceIndex, transition.selector)) {
        ...
        reactions.push(reactionFor(transition, destination?.id));
