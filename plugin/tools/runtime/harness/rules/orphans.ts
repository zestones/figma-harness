...
const rule: AuditRule = {
  ...
  run({ created }) {
    ...
    for (const node of created) {
      ...
      // audit evidence without retaining destroyed document graphs.
      if (node._everAttached || !node.children.length || node.type !== 'FRAME') continue;
      const key = node.name + '  (' + node.children.length + ' children, ' +
        node.children.map((child) => child.name).slice(0, 3).join(', ') + ')';
      orphans.set(key, (orphans.get(key) || 0) + 1);
    }

    console.log('\n--- orphans ---');
    if (!orphans.size) {
      console.log('  every frame with children is attached to a page');
      return 0;
    }
    let issues = 0;
