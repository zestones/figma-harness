...
export function advanceTable(family: string, style: string): AdvanceTable {
  const name = BUNDLED_FAMILIES[family] ? family : DEFAULT_FAMILY;
  const weights = BUNDLED_FAMILIES[name].weights;
  ...
  if (!table) {
    ...
    if (!table.size) throw new Error('bundled font metrics are missing for ' + key + '; run npm ci');
