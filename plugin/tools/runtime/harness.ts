...
import { measure } from './harness/text-metrics.ts';
import type { AuditRuntime } from './harness/rules/types.ts';
import type { StressRuntime } from './harness/stress.ts';
import { BUNDLE_FILE, BUNDLE_GLOBAL_NAME } from './bundle.ts';

const fs = require('node:fs') as typeof import('node:fs');
const path = require('node:path') as typeof import('node:path');
const vm = require('node:vm') as typeof import('node:vm');
const { runAudit } = require('./harness/audit.ts') as typeof import('./harness/audit.ts');
const { stress: runStress } = require('./harness/stress.ts') as {
...
export function createHarness(options: HarnessOptions = {}): Harness {
  ...
  vm.runInContext(
    options.code ?? fs.readFileSync(BUNDLE_FILE, 'utf8'),
    context,
    { filename: 'code.js' },
  );
