...
import { labPageName } from './harness/contract.ts';
import { createMockNodeFactory } from './harness/figma-mock/node-factory.ts';
import { nodeSignature } from './signature.ts';
...
async function document(labOnly = false): Promise<void> {
  ...
  const { createSignatureFromBuiltPages } = await import('./design-signature.ts');
...
async function main(): Promise<void> {
  ...
  if (!/\/app\.slice\/figma-harness-check-[\w-]+\.service/.test(group)) {
    throw new Error('Use npm run isolated -- diagnose:memory <nodes|assertion|document|lab> [renders/output.json]');
  }
  ...
  if (output) {
    ...
    if (!filename.startsWith(path.join(ROOT, 'renders') + path.sep)) throw new Error('Output must be inside renders/');
