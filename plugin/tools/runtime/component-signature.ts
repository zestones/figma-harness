...
import type { MockNode } from './harness/figma-mock/types.ts';
import {
...
const BASELINE_FILE = path.join(__dirname, 'component-baseline.json');
...
createSignature().then((signature) => {
  ...
  if (!fs.existsSync(BASELINE_FILE)) {
    console.error(missingBaselineMessage(path.basename(BASELINE_FILE), 'design:components'));
    process.exitCode = 1;
    return;
  }
