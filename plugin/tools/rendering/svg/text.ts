import { DEFAULT_FAMILY, fontWeight } from '../../core/bundled-fonts.ts';
import { wrapLines } from '../../runtime/harness/text-metrics.ts';
import {
  escapeXml,
  paintFill,
} from './paint.ts';
...
export function drawText(
  ...
  const family = node.fontName?.family || DEFAULT_FAMILY;
  ...
  for (let index = 0; index < shown.length; index++) {
    output.push(
      ...
      + '" font-family="' + escapeXml(family) + ', ' + DEFAULT_FAMILY + ', sans-serif"'
      + ' font-size="' + size
      + '" font-weight="' + weight
