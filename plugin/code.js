/* @generated by packages/harness/src/bundle/build.ts — do not edit; run pnpm build. */
"use strict";
var FigmaHarnessPlugin = (() => {
          ...
          return {
            ...
            transition: action.transition ? {
              ...
              easing: {
                ...
                ...action.transition.easing.easingFunctionCubicBezier ? { easingFunctionCubicBezier: { ...action.transition.easing.easingFunctionCubicBezier } } : {}
              }
            } : null
          };
        })
  ...
  var strutForRow = function(row, nodes) {
    if (!nodes || typeof nodes.length !== "number" || typeof nodes === "number") {
      throw new Error("strutForRow(row, nodes): expected an array of nodes, got " + typeof nodes + ' on "' + row.name + '". The gap is read off the row.');
    }
    const gap = row.itemSpacing || 0;
    const childCount = nodes.length + 1;
    let fixedWidth = 0;
    for (const node of nodes) fixedWidth += node.width;
    return Math.max(1, row.width - (row.paddingLeft || 0) - (row.paddingRight || 0) - (childCount - 1) * gap - fixedWidth);
  };
  ...
  var spread = function(row, width) {
    const children = row.children.filter(function(child) {
      return "layoutPositioning" in child && "resize" in child && child.layoutPositioning !== "ABSOLUTE";
    });
    if (!children.length) return row;
    let usedWidth = (row.paddingLeft || 0) + (row.paddingRight || 0) + (children.length - 1) * (row.itemSpacing || 0);
  ...
  function dim(name) {
    ...
    if (!reference) {
      const fields = [];
      if (token2.scopes.includes("WIDTH_HEIGHT")) fields.push(...SIZE_FIELDS);
      if (token2.scopes.includes("GAP")) fields.push(...SPACE_FIELDS);
      if (token2.scopes.includes("CORNER_RADIUS")) fields.push(...RADIUS_FIELDS);
      reference = Object.freeze({ variable: name, value: token2.value, fields: Object.freeze(fields) });
  ...
  var STATE_FAMILIES = Object.freeze(["neutral", "accent", "success", "attention", "severe", "danger", "done"]);
  var EMPHASIS = STATE_FAMILIES.map((kind) => STATUS[kind].emphasis);
  ...
  var STATE_FAMILIES2 = Object.freeze({
    open: "open",
    closed: "closed",
    done: "done",
    draft: "draft",
  ...
  var stateLabel = async function(options) {
    const family3 = STATUS[STATE_FAMILIES2[options.status]];
  ...
  var inlineMessage = async function(variant2, text, width) {
    ...
    const row = await createFrame({ name: "inline-message/" + variant2, dir: "H", w: width, gap: dim("base/size/8"), align: "CENTER" });
    row.appendChild(icon(glyph2, ink, 16));
    row.appendChild(await createText({ style: "body/medium", text, color: ink, w: width - 24 }));
    row.setPluginData("aria.live", variant2 === "critical" ? "assertive" : "polite");
    return row;
  };
  ...
  var dataTable = async function(options) {
    ...
    const row = async function(name, height, header) {
      const frame = await createFrame({
        name,
        dir: "H",
        w: options.w,
  ...
  var colorRow = async function(width, token2, role2) {
    ...
    const row = await createFrame({ name: "c/" + token2, dir: "H", w: width, h: 24, gap: dim("base/size/8"), align: "CENTER" });
    row.appendChild(await chip(token2));
    row.appendChild(await createText({ style: "code/small", text: token2, w: COLOR_NAME_W, truncate: true }));
    row.appendChild(await createText({
      style: "code/small",
  ...
  var sheetC1 = defineSheet({ code: "C1", group: "Components", title: "Buttons", build: async () => {
    ...
    for (const [variant2, text, use] of VARIANTS) {
      const row2 = await createFrame({ name: "row/" + variant2, dir: "H", w: sh.w, h: 64, gap: 16, align: "CENTER" });
  ...
  var LABEL_VARIANTS = ["default", "secondary", "accent", "success", "attention", "severe", "danger", "done"];
  var PEOPLE = [
  ...
  var sheetC2 = defineSheet({ code: "C2", group: "Components", title: "Labels and identity", build: async () => {
    ...
    for (const variant2 of LABEL_VARIANTS) labels.body.appendChild(await label({ text: variant2, variant: variant2 }));
    for (const variant2 of LABEL_VARIANTS.slice(0, 3)) labels.body.appendChild(await label({ text: variant2 + " large", variant: variant2, size: "large" }));
    ...
    for (const size of [16, 20, 24, 32, 40]) people.body.appendChild(await avatar({ ...PEOPLE[0], size, series: size % 4 }));
  ...
  var createRelay = function() {
    return {
      ...
      project: "storefront",
      viewerId: "u1",
      people: PEOPLE2.map((person) => ({ ...person })),
  ...
  var LABEL_VARIANTS2 = Object.freeze({
    bug: "danger",
    feature: "accent",
    performance: "success",
    checkout: "done",
  ...
  var checksSummary = async function(release) {
    ...
    row.setPluginData("aria.accessible-name", passed + " of " + total + " checks passed" + (running2 ? ", " + running2 + " running" : "") + (failed ? ", " + failed + " failed" : ""));
    return row;
  };
  var releaseRow = async function(data, release, width) {
    ...
    const labels = [];
    for (const name of release.labels) labels.push(await label({ text: name, variant: LABEL_VARIANTS2[name] || "default" }));
  ...
  var releaseList = async function(data, releases, width, header) {
    const rows = [];
    ...
    return box({
      ...
      header: {
        title: header.title,
        count: header.count == null ? void 0 : await counterLabel({ count: header.count }),
  ...
  var checkRow = async function(check, width) {
    ...
    row.appendChild(await createStrut(strutForRow(row, [row.children[0], name, summary, duration, details]), 1));
  ...
  var FIELD_WIDTH = 440;
  var dangerRow = async function(title, body, action, width) {
    const row = await createFrame({
      name: "danger/" + action,
      dir: "H",
      w: width,
