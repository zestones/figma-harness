...
- consume visual vocabulary only through the design system's package entry (`@figma-harness/primer`);
...
- treat `packages/`, `design-systems/`, `plugin/`, the app's `src/index.ts` and `src/signatures.ts`, tooling, ADRs, and signature baselines as protected.
