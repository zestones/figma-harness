...
3. Choose the smallest owning layer: foundation, primitive, component, or pattern. Figma mechanics that know no token belong in `packages/engine/`, which is a `STRUCTURAL_MAINTENANCE` change.
4. Add the supported export to the design system's `src/index.ts` only when page authors should use it. Internal helpers stay private.
5. Demonstrate the capability on the appropriate design-system sheet and add focused tests in the package's `tests/`. A new component family is added to `src/components/inventory.ts` so the inventory audit requires a specimen.
...
Primer's tokens and icons are generated. List a new one in `design-systems/primer/generators/tokens/catalog.ts` or `generators/icons/catalog.ts` and regenerate it; never type a Primer value by hand. Every colour token must appear as a `c/<token>` row on the Colour sheets (A1–A6), and the colour-inventory audit fails on a missing or unknown row. When you add or remove a colour, or update Primer:

- declare the pairs it must pass in `CONTRAST_PAIRS` (`src/foundations/audit.ts`);
- give a state or data colour its family in `src/foundations/semantics.ts`, so `CATEGORICAL_AUDIT` checks its separation under simulated colour-vision deficiency;
